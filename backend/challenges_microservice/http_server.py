...
async def whoami(session_id: str):
    async with aiohttp.ClientSession() as session:
        async with session.get(
            "http://users_microservice:8080/users/whoami",
    ...
    async with sql_client.cursor() as cur:
        await cur.execute(
            ...
            (
                ...
                datetime.datetime.fromtimestamp(request_body.end_date).replace(
                    tzinfo=datetime.timezone.utc
                ),
...
async def register_for_challenge(
    challenge_id: int, session_id: str = Cookie(None), sql_client=Depends(get_db)
):
    """
...
async def get_challenge_registrations(challenge_id: int, sql_client=Depends(get_db)):
    ...
    async with sql_client.cursor() as cur:
        await cur.execute(
            "SELECT username FROM submissions WHERE challenge_id=%s", (challenge_id)
        )
        query_result = await cur.fetchall()
    if not query_result:
        return []
...
async def link_project(
    submission_id: int, request_body: LinkProjectRequest, sql_client=Depends(get_db)
):
    """
    ...
    async with sql_client.cursor() as cur:
        await cur.execute(
            "UPDATE submissions SET code_repository = %s WHERE submission_id = %s",
            (request_body.link, submission_id),
        )
...
async def vote(
    ...
    async with sql_client.cursor() as cur:
        ...
        await cur.execute(
            "SELECT vote_id FROM votes WHERE voter_username=%s", (voter_username)
        )
        if await cur.fetchone():
            raise HTTPException(
                status_code=400, detail="You already voted for this submission"
            )
        await cur.execute(
            "INSERT IGNORE INTO votes (submission_id, voter_username, weight) VALUES (%s,%s,%s)",
            (submission_id, voter_username, request_body.weight),
        )
    return 200
...
async def get_challenge_submissions(challenge_id: int, sql_client=Depends(get_db)):
    ...
    async with sql_client.cursor() as cur:
        await cur.execute(
            "SELECT submissions.submission_id, username, SUM(votes.weight) FROM submissions LEFT JOIN votes ON submissions.submission_id=votes.submission_id WHERE challenge_id=%s GROUP BY submissions.submission_id LIMIT 10",
            (challenge_id),
        )
        query_result = await cur.fetchall()
    if not query_result:
